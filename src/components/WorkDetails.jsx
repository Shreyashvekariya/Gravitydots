...
const CATEGORIES = [
    { name: 'Branding', bgColor: '#D53F52', tagline: '"8 out of 10 Customers don\'t remember products, they remember brands"' },
    { name: 'Graphic Design', bgColor: '#F5974E', tagline: '"Visual content is processed 60,000× faster than text"' },
    { name: 'Social Media Management', bgColor: '#5F94C9', tagline: '"Over 70% of customers check a brand\'s social media before buying"' },
    { name: 'Performance Marketing', bgColor: '#1800AD', tagline: '"Businesses earn an average of 2X-5X ROAS with well optimized campaigns"' },
    { name: 'Search Engine Optimization (SEO)', bgColor: '#7ED957', tagline: '"Over 60% of website traffic often comes from organic search"' },
    { name: 'Influencer Marketing', bgColor: '#5170FF', tagline: '"Nearly 70% of consumers trust creator recommendations over traditional ads"' },
    { name: 'Website Development', bgColor: '#00B8BF', tagline: '"88% of users don\'t return after a poor website experience"' },
    { name: 'Whatsapp Marketing', bgColor: '#FF751F', tagline: '"WhatsApp messages can achieve open rates above 90%"' },
    { name: 'Video Editing', bgColor: '#8E44AD', tagline: '"Videos generate up to 2.5X more engagement than static creatives"' },
];
...
// ── Non-eager glob for WhatsApp Marketing ─────────────────────────────────
const whatsappFilesLazy = import.meta.glob('../assets/Work/whatsapp/*.{webp,jpg,jpeg,png,mp4}');

const whatsappLoaders = [];
Object.keys(whatsappFilesLazy).forEach(glbPath => {
    const parts = glbPath.split('/');
    const fileName = parts.pop();
    whatsappLoaders.push({
        glbPath,
        importFn: whatsappFilesLazy[glbPath],
        fileName,
        type: fileName.endsWith('.mp4') ? 'video' : 'image',
    });
});

whatsappLoaders.sort((a, b) => {
    const na = parseFloat(a.fileName.match(/[\d.]+/)?.[0] || 0) || 0;
    const nb = parseFloat(b.fileName.match(/[\d.]+/)?.[0] || 0) || 0;
    return na - nb;
});

const whatsappMarketingProjects = whatsappLoaders.length > 0 ? [{
    id: 'whatsapp-marketing-0',
    title: 'WhatsApp Marketing',
    category: 'WHATSAPP MARKETING',
    loaders: whatsappLoaders,
...
const generateProjects = (category) => {
    ...
    if (category === 'Whatsapp Marketing' && whatsappMarketingProjects.length > 0) return whatsappMarketingProjects;
...
const LazyPopupImage = memo(({ loader, alt, index }) => {
    ...
    // Play videos only while visible so off-screen clips don't keep decoding
    useEffect(() => {
        if (!mediaData || mediaData.type !== 'video') return;
        const vid = containerRef.current;
        if (!vid) return;
...
// ── Downscale a full-res image to a small thumbnail texture off the main thread.
// Huge source images rasterised into small cards are the main grid scroll-lag
// source; shrinking the texture keeps compositing cheap. Falls back to the
// original URL on any failure so the UI never breaks.
const THUMB_TARGET_WIDTH = 700;
const downscaleImage = async (url) => {
    try {
        if (typeof createImageBitmap !== 'function') return url;
        const res = await fetch(url);
        const blob = await res.blob();
        const bitmap = await createImageBitmap(blob);
        if (bitmap.width <= THUMB_TARGET_WIDTH) {
            bitmap.close?.();
            return url;
        }
        const scale = THUMB_TARGET_WIDTH / bitmap.width;
        ...
        return outBlob ? URL.createObjectURL(outBlob) : url;
    } catch {
        return url;
    }
};
            ...
            ([entry]) => {
                if (entry.isIntersecting && !loadedRef.current) {
                    ...
                    if (project.loaders && project.loaders.length > 0) {
                        ...
                        loader.importFn().then(async (mod) => {
                            const rawUrl = mod.default || mod;
                            if (loader.type === 'video') {
                                if (!cancelled) setThumb({ url: rawUrl, type: 'video' });
                                return;
                            }
                            const smallUrl = await downscaleImage(rawUrl);
                            if (cancelled) {
                                if (smallUrl !== rawUrl) URL.revokeObjectURL(smallUrl);
                                return;
                            }
                            if (smallUrl !== rawUrl) blobUrlRef.current = smallUrl;
                            setThumb({ url: smallUrl, type: loader.type });
                        });
                    }
                }
            },
...
const WorkDetails = () => {
    ...
    // Keyboard lock + pause Lenis so the popup scrolls natively (no double-scroll jank)
    useEffect(() => {
        ...
        // Stop Lenis smooth scroll while the modal is open; the modal body uses
        // data-lenis-prevent + native overflow scrolling instead.
        ...
        <div className="work-details-wrapper">
            <section className="work-details-page">
                <div className="work-details-container">
                    ...
                    <p className="work-details-tagline" key={activeCategory}>
                        {CATEGORIES.find(c => c.name === activeCategory)?.tagline}
                    </p>
                    {activeCategory === 'Influencer Marketing' ? (
                        ...
                        activeVideoCategory ? (
                            <div className="work-details-video-category-view">
                                <main className="work-details-reels-grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px' }}>
            ...
            {selectedProject && (
                <div className="wd-popup-overlay" onClick={closePopup}>
                    <div className="wd-popup-content" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="wd-popup-scroll-body" data-lenis-prevent>
